...
.PHONY: dev-up dev-up-kafka dev-down load-sample replay-events build-canonical switch-alias build-suggest test-python test-api lint-api

dev-up:
	$(COMPOSE) -p $(PROJECT) up -d elasticsearch kibana

dev-up-kafka:
	$(COMPOSE) -p $(PROJECT) -f docker-compose.yml -f docker-compose.kafka.yml up -d elasticsearch kibana redpanda redpanda-console
